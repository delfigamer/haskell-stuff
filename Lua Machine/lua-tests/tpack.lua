...
do
  local lnum = 0x706050403020100
  local invlnum = (lnum ~ 0xffffffffffffffff) + 1
  local s = pack("<j", lnum)
  ...
  for i = sizeLI + 1, NB do
    ...
    assert(unpack("<I" .. i, s .. ("\0"):rep(i - sizeLI)) == invlnum)
  end
end
...
assert(unpack("<J", pack("<j", -1)) == 0xffffffffffffffff)
